...
export function Register() {
    const [formData, setFormData] = useState({ name: '', email: '', password: '', confirmPassword: '', role: 'user', phone: '' });
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            const user = await registerWithEmail(formData.email, formData.password, formData.name, formData.role);
            setUser(user);
        ...
        <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4 py-12 relative w-full">
            ...
            <div className="relative z-10 w-full max-w-md">
                <div className="text-center mb-8">
                    ...
                    <p className="text-slate-500 mt-2">Join BookMyPandit and start your spiritual journey</p>
                </div>
                ...
                <div className="glass-card rounded-3xl p-8">
                    ...
                    <form onSubmit={handleRegister} className="space-y-4">
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <button type="button" onClick={() => setFormData({ ...formData, role: 'user' })}
                                    className={`p-3 rounded-xl border text-sm font-medium transition-all flex flex-col items-center gap-2 ${formData.role === 'user' ? 'border-primary bg-primary/5 text-primary shadow-sm' : 'border-slate-200 text-slate-500 hover:border-slate-300'}`}>
                                    <span className="material-symbols-outlined">person</span>
                                    Book Pujas
                                </button>
                                <button type="button" onClick={() => setFormData({ ...formData, role: 'pandit' })}
                                    className={`p-3 rounded-xl border text-sm font-medium transition-all flex flex-col items-center gap-2 ${formData.role === 'pandit' ? 'border-primary bg-primary/5 text-primary shadow-sm' : 'border-slate-200 text-slate-500 hover:border-slate-300'}`}>
                        ...
                        <button type="submit" disabled={isLoading}
                            ...
                            {isLoading ? <span className="size-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></span> : (
                ...
                <p className="text-center text-sm text-slate-500 mt-6">
                    ...
                    <Link to="/login" className="text-primary font-semibold hover:text-orange-600 transition-colors">Sign in</Link>
                </p>
            </div>
        </div>
    );
